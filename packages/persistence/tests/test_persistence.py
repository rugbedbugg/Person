...
def test_older_journals_still_read_and_new_events_cannot_backdate_themselves() -> None:
    ...
    assert EvidenceEvent.from_json(search).schema_version == "person-evidence-v8"
    ...
    for kind in ("memory_encoded", "memory_recalled"):
        ...
        assert EvidenceEvent.from_json(memory).schema_version == "person-evidence-v8"
    ...
    assert EvidenceEvent.from_json(learned).schema_version == "person-evidence-v8"
    with pytest.raises(EvidenceError):
        EvidenceEvent.from_json({**learned, "schema_version": "person-evidence-v7"})

    affect = {**document, "type": "affect_appraised", "payload": {}}
    assert EvidenceEvent.from_json(affect).schema_version == "person-evidence-v8"
    with pytest.raises(EvidenceError):
    ...
    for kind in ("project_started", "project_changed"):
        ...
        assert EvidenceEvent.from_json(project).schema_version == "person-evidence-v8"
    ...
    for kind in ("place_formed", "place_visited"):
        ...
        assert EvidenceEvent.from_json(place).schema_version == "person-evidence-v8"
