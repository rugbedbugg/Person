...
| `src/report.js` atomic summary writing with temp file and rename                                                                                                                       | `apps/node-runtime/src/reporting/episode-report.ts`, `apps/node-runtime/src/runtime/placement-ledger.ts`, `packages/persistence/python/person_persistence/snapshot.py` | Same technique, applied to every durable write on both sides.                                                                                                                                                                                                                                                       |
...
- `src/resume.js` inventory reconciliation, which encoded a V1 recovery
  protocol. Ownership is now tracked as provenance records in `PlacementLedger` (then `WorldMemory`);
...
- Home comes from `PlacementLedger` (then `WorldMemory`), as it always did. Nothing in the CLI or the
